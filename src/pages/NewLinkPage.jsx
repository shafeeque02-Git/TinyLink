...
export default function NewLinkPage({ onCreate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTarget(''); setDescription(''); setTags(''); setCode(''); setCustom(false)
    } catch (err) {
  ...
  return (
    <div>
      <h2 style={{marginTop:0}}>Create New Short Link</h2>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{display: 'flex', justifyContent: 'flex-end', gap: '8px'}}>
          <button type="button" onClick={() => { setTarget(''); setDescription(''); setTags(''); setCustom(false); setCode('') }}>Reset</button>
